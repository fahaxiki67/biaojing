...
当前交付为源码运行版，需要 Python 3.10 或更高版本。本轮实际安装环境为 Apple Silicon Mac、Python 3.14.7；Intel Mac 和 Windows 尚未真机验收。
...
发布新版本时，将 `app/biaojing/__init__.py` 中的 `VERSION` 与 Release 标签保持一致（例如 `0.1.1` / `v0.1.1`）。发布 Release 后，`.github/workflows/publish-update.yml` 会生成并上传更新包。普通 Git 推送不会直接触发安装；要让已安装副本更新，请发布一个新的 Release。当前更新器面向源码运行版，只更新程序代码，不是完整安装器；Mac 以外的平台、特别是 Windows，尚未真机验收。
...
有文字输出不代表数字、手写内容或表格结构准确。当前尚未实现扫描表格的自动行列还原。PDF 仍以原生文字优先；文字和大幅扫描图混排的页面需要额外核对是否提取完整。
...
当前单文件上传上限为 128 MB。三页及以上的直接 PDF 上传和所有 PDF 待 OCR 重试会在后台逐页运行，页面显示当前页并可取消；取消在当前页处理结束后生效，已完成页保留，未处理页明确标为待处理，之后可重试。同一工作台一次只运行一个长任务。少于三页的 PDF 上传和 ZIP（包括 ZIP 内 PDF）仍同步处理。不要把“已处理”理解为“全部识别成功”，应查看每份文件的 `partial`、待 OCR 页数和原因。
...
GitHub 仓库仅发布程序源码、安装与启动脚本、依赖清单及合成测试样本。项目资料、审计报告、参考代码副本和本机运行数据不属于公开发布内容。仓库尚未声明开源许可证；Windows 安装包和 Windows 真机测试仍待后续完成。
